...
- A recording that is interrupted now says so. Playwright closes every browser it launched when the process is signalled but leaves the process running, so a walkthrough killed part-way failed as a broken recording step with Playwright's generic "Target page, context or browser has been closed", and left a debug artifact describing the wrong problem. The interruption is now named, with the phase it happened in and what to do about it; the debug artifact still records the original error and its stack.
