...
describe("WAV validation and sealing", () => {
  ...
  test("enforces the streaming output bound and cancellation", async () => { const root = await mkdtemp(join(tmpdir(), "kokoro-wave-bound-")); const source = join(root, "in.wav"); await writeFile(source, wav()); await expect(sealWaveFile(source, join(root, "bounded.wav"), { maxOutputBytes: 45 })).rejects.toThrow(/safety limit/); const controller = new AbortController(); controller.abort(new DOMException("stop sealing", "AbortError")); await expect(sealWaveFile(source, join(root, "cancelled.wav"), { signal: controller.signal })).rejects.toThrow("stop sealing"); });
});
